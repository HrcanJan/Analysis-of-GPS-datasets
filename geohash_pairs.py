...
def split_to_single_day(in_file, out_file):
    ...
    for index, row in tqdm(df.iterrows()):
        ...
        if start.date() == end.date():
            ...
            new_rows.append(row.copy())  # Append the modified row for the first day

            # Handle full days in between, if any
            current = start + pd.Timedelta(days=1)
            current = current.replace(hour=0, minute=0, second=0)  # Start of the next day
